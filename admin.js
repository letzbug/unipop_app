...
function renderUsersFiltered(){
  ...
  const list=!q?adminUsers:adminUsers.filter(u=>
    String(u.trainer_name||"Administrateur UniPop Go").toLowerCase().includes(q)||
    String(u.email||"").toLowerCase().includes(q)
  );
  $("#userCount").textContent=`${adminUsers.length} accès`;
  $("#usersList").innerHTML=list.map(renderUser).join("")||"<p>Aucun accès correspondant.</p>";

  document.querySelectorAll(".toggle-user").forEach(b=>b.onclick=async()=>{
    const card=b.closest(".user-card"),id=card.dataset.id;
    const row=adminUsers.find(x=>x.id===id);
    if(!row)return;
  ...
  document.querySelectorAll(".delete-user").forEach(b=>b.onclick=async()=>{
    const card=b.closest(".user-card"),id=card.dataset.id;
    const row=adminUsers.find(x=>x.id===id);
    if(!row||!confirm(`Supprimer l'accès de ${row.trainer_name||row.email} ?\n\nLe compte est archivé afin de pouvoir être réautorisé plus tard sans casser son accès Supabase.`))return;
    const {error}=await sb.from("trainer_access")
...
async function loadUsers(){
  const {data,error}=await sb.from("trainer_access")
    .select("id,email,trainer_name,role,active,auth_user_id,created_at,updated_at,deleted_at")
    .is("deleted_at",null)
    .order("trainer_name",{ascending:true,nullsFirst:true});
...
$("#addTrainer").onclick=async()=>{
  ...
  const {data:existing,error:findError}=await sb.from("trainer_access")
    .select("id,auth_user_id,deleted_at")
    .ilike("email",email)
    .maybeSingle();
  if(findError){setStatus($("#addStatus"),findError.message,"error");return;}

  let error=null;
  if(existing){
    ({error}=await sb.from("trainer_access")
      .update({trainer_name:trainerName,email,role,active:true,deleted_at:null})
      .eq("id",existing.id));
  }else{
    ({error}=await sb.from("trainer_access").insert({trainer_name:trainerName,email,role,active:true}));
  }

  if(error){setStatus($("#addStatus"),error.message,"error");return;}
  $("#trainerName").value="";$("#trainerEmail").value="";
  setStatus($("#addStatus"),existing
    ?"Accès réautorisé. Le formateur peut se reconnecter ou utiliser « Première connexion » pour recevoir un nouveau lien."
    :"Accès autorisé. Le formateur utilise « Première connexion » avec son e-mail.","success");
  await loadUsers();
};
showAdminIfAllowed();

$("#userSearch")?.addEventListener("input",renderUsersFiltered);
